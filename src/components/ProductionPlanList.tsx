  ...
  if (plan.items.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <p className="text-gray-500 text-sm">No items in the plan yet. Add items from the calculator on the left.</p>
      </div>
    );
  }
          ...
          return (
            <div
              ...
              className={`border rounded-lg p-4 ${
                item.completed ? "bg-gray-50 border-gray-300 opacity-60" : "bg-white border-gray-200"
              }`}
            >
                  ...
                  <div className="space-y-2">
                    {materials.map((material) => {
                      ...
                      return (
                        <div
                          key={material.item}
                          className={`flex items-center gap-3 p-2 rounded ${isComplete ? "bg-green-50" : "bg-gray-50"}`}
                        >
                          <input
                            type="checkbox"
                            checked={isComplete}
                            onChange={() =>
                              onUpdateMaterialProgress(item.id, material.item, isComplete ? 0 : progress.required)
                            }
